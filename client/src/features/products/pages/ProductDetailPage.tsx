    ...
    const fetchData = async () => {
      try {
        ...
        const related = allProducts
          .filter(
            (p: Product) =>
              (p.category as any)?.name === (currentProduct.category as any)?.name &&
  ...
  return (
    <div className="min-h-screen bg-[#0f0f12] text-white font-sans pb-20">
      <div className="container mx-auto px-4 mt-6 max-w-6xl">
          ...
          <div className="w-full md:w-2/5 flex flex-col">
            ...
            <div className="mt-8 pt-6 border-t border-white/10">
              ...
              <div className="flex gap-3 opacity-60">
                {[['📄', 'Manual'], ['🔌', 'Cable'], ['🛡️', 'Warranty']].map(([icon, label]) => (
                  <div key={label} className="w-12 h-12 bg-white/5 border border-white/10 rounded flex flex-col items-center justify-center text-[8px] gap-1">
        ...
        <div className="mb-16">
          <h3 className="text-xl font-bold mb-6 flex items-center gap-3">
            <span className="w-1.5 h-8 bg-gray-600 rounded-full block"></span> รีวิวจากลูกค้า
          </h3>
        ...
        <div className="border-t border-white/10 pt-10">
          <h3 className="text-2xl font-bold mb-8 text-white flex items-center gap-2">
            ...
            <span className="text-red-600 text-sm font-normal cursor-pointer hover:underline">ดูทั้งหมด →</span>
          </h3>
            ...
            {relatedProducts.length > 0 ? (
              relatedProducts.map((rel) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h4 className="font-bold text-sm text-white line-clamp-1 group-hover:text-red-500 transition">{rel.name}</h4>
                    <p className="text-gray-500 text-xs mt-1 mb-2">
                      {typeof rel.category === 'object' ? (rel.category as any)?.name : rel.category}
                    </p>
                    <div className="flex justify-between items-center">
                      <p className="text-red-500 font-bold">฿{Number(rel.price).toLocaleString()}</p>
                      <button
                        onClick={(e) => { e.stopPropagation(); addToCart(rel.id, 1).catch(console.error); }}
